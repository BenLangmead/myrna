...
# push.sh
#
# Run from Myrna root directory (i.e. sh emr/util/push.sh X1.Y1.Z1
...
# an S3 subdirectory corresponding to one version of Myrna to
...
s3cmd -c $S3CFG \
	...
	s3://myrna-emr/$VERSION_OLD/ \
	s3://myrna-emr/$VERSION_NEW
...
for i in 32 64 ; do
	s3cmd -c $S3CFG \
		...
		put \
		bin/linux$i/bowtie \
		s3://myrna-emr/$VERSION_NEW/bowtie$i
done
